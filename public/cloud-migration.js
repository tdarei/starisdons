...
class CloudMigration {
    ...
    init() {
        this.trackEvent('cloud_mig_initialized');
    }
    ...
    createMigration(migrationId, migrationData) {
        const migration = {
            ...
            assessmentId: migrationData.assessmentId || null,
    ...
    trackEvent(eventName, data = {}) {
        try {
            if (window.performanceMonitoring) {
                window.performanceMonitoring.recordMetric(`cloud_mig_${eventName}`, 1, data);
            }
        } catch (e) { /* Silent fail */ }
    }
}
...
if (typeof window !== 'undefined') {
    window.cloudMigration = new CloudMigration();
}
