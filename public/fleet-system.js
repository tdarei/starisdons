...
class FleetManager {
    ...
    renderFleetList() {
        ...
        this.game.ships.forEach(ship => {
            ...
            let actionButtons = `
                <button class="ep-sys-btn" style="flex:1; font-size:0.9em; padding:8px; background:rgba(56, 189, 248, 0.2); border-color:#38bdf8; color:#38bdf8;" onclick="window.game.fleetManager.startMission(window.game.ships.find(s=>s.id=='${ship.id}'), 'scout_patrol')">Patrol (10s)</button>
                <button class="ep-sys-btn" style="flex:1; font-size:0.9em; padding:8px; background:rgba(239, 68, 68, 0.2); border-color:#ef4444; color:#ef4444;" onclick="window.game.fleetManager.scrapShip('${ship.id}')">Scrap</button>
            `;
