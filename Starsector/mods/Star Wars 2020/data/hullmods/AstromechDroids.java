...
public class AstromechDroids extends BaseHullMod {
	...
	public void applyEffectsBeforeShipCreation(HullSize hullSize, MutableShipStatsAPI stats, String id) {
		...
		stats.getOverloadTimeMod().modifyMult(id, 1f - RECOVERY_BONUS * 0.01f);
...
//		stats.getSuppliesPerMonth().modifyPercent(id, LOGISTICS_PENALTY);
	...
	public String getDescriptionParam(int index, HullSize hullSize) {
		...
		if (index == 1) return "" + (int) RECOVERY_BONUS + "%";
		if (index == 2) return "" + (int) CR_RECOVERY_BONUS + "%";
		return null;
	}
