...
public class DomresSeededFleetManager extends SeededFleetManager {

	
	public static class DomresFleetInteractionConfigGen implements FIDConfigGen {
...
//					}
	...
	public DomresSeededFleetManager(StarSystemAPI system, int minFleets, int maxFleets, int minPts, int maxPts, float activeChance) {
		super(system, 1f);
		this.minPts = minPts;
		this.maxPts = maxPts;
	...
	protected CampaignFleetAPI spawnFleet(long seed) {
		...
		initDomresFleetProperties(random, fleet, dormant);
...
//				fleet.setLocation(loc.x, loc.y);
			...
			fleet.addScript(new DomresAssignmentAI(fleet, system, null));
		}
	...
	public static void initDomresFleetProperties(Random random, CampaignFleetAPI fleet, boolean dormant) {
...
//			fleet.addAssignment(FleetAssignment.HOLD, null, 1000000f, "dormant");
		...
		addDomresInteractionConfig(fleet);
	}
	
	public static void addDomresInteractionConfig(CampaignFleetAPI fleet) {
		fleet.getMemoryWithoutUpdate().set(MemFlags.FLEET_INTERACTION_DIALOG_CONFIG_OVERRIDE_GEN, 
				   new DomresFleetInteractionConfigGen());
	}
