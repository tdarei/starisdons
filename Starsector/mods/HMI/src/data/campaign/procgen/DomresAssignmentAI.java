...
public class DomresAssignmentAI implements EveryFrameScript {
	...
	public DomresAssignmentAI(CampaignFleetAPI fleet, StarSystemAPI homeSystem, SectorEntityToken source) {
		this.fleet = fleet;
	...
	protected void giveInitialAssignments() {
		...
		if (playerInSameLocation && (float) Math.random() < 0.1f && source != null) {
			...
			SectorEntityToken target = data.campaign.procgen.DomresSeededFleetManager.pickEntityToGuard(new Random(), homeSystem, fleet);
			if (target != null) {
	...
	protected void pickNext() {
		...
		if (!standDown) {
			SectorEntityToken target = DomresSeededFleetManager.pickEntityToGuard(new Random(), homeSystem, fleet);
			if (target != null) {
